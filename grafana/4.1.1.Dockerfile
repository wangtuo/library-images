...
RUN apt-get update && \
    ...
    apt-get clean && \
    curl http://okxq98xn6.bkt.clouddn.com/grafana/grafana_4.1.1-1484211277_amd64.deb > /tmp/grafana.deb && \
...
COPY grafana.4.1.1.ini /etc/grafana/grafana.ini
COPY start.4.1.1.sh /start.sh
